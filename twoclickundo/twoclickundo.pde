...
boolean [] errors = new boolean[trialCount];
...
void mousePressed()
{
  ...
  if(mouseButton == RIGHT)
  {
    ...
    if (clicks > 0) {
      clicks--;
      int removeX = clickx.remove(0);
      int removeY = clicky.remove(0);
      ...
      if (clicks % 2 == 0) {
        ...
        c2Y = -1;
        trialIndex--;
        if (errors[trialIndex]) errorCount--;
        errors[trialIndex] = false;
      }
    }
